...
8. Сборщик мусора собирает объекты только из кучи, он делает это путём принципа достижимости, т.е. строится граф достижимости каждого объекта, после этого все НЕдостижимые (неиспользуемые) объекты удаляются. Из стека все фреймы и значимые типы (переменные) удаляются автоматически сразу после выполнения.
